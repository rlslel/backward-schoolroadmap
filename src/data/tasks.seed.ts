...
//    시트에 실제 날짜를 적어 덮어쓴다.
// 4. offsetDays 는 규정이 아니라 제안값이다. 어느 지침에서 가져온 숫자가 아니라
//    "견적 3개 받고 결재 올리려면 이 정도 걸린다" 같은 일반적인 추론으로 정한 것이다.
//    학교마다 다르므로 정답이 없다. 화면에서도 규정처럼 보이지 않게 표시할 것.
//    현직 교사가 검토해 고친 항목에는 아래처럼 표시를 남긴다.
//      // 검토 완료: 3개 업체 견적 회신이 늦어 D-35 로 늘림 (2026-09-06)
//
// ── basis 가 비어 있다는 것의 뜻 ──────────────────────────────────
